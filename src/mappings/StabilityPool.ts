...
import {
  UserDepositChanged,
  FrontEndRegistered,
  FrontEndTagSet, WSTETHGainWithdrawn,
...
function swapTmpDepositUpdate(valueToSetIfNull: BigInt): BigInt | null {
  ...
  if (!tmpDepositUpdate) {
    global.tmpDepositUpdate = null;
  } else {
    global.tmpDepositUpdate = valueToSetIfNull;
  }
  global.save();
...
export function handleWSTETHGainWithdrawn(event: WSTETHGainWithdrawn): void {
  ...
  withdrawCollateralGainFromStabilityDeposit(
    ...
    event.params._WSTETH,
    event.params._SIMLoss
  );
